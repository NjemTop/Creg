                ...
                .then(data => {
                    ...
                    if (data[0].tech_information !== null) {
                        ...
                        function showVersionFormatInfo() {
                            infoShowToast('Пример формата версии: 2.45.35301.26201');
                        }

                        // Функция для вывода информационного сообщения о формате версии
                        function showMDMFormatInfo() {
                            infoShowToast('Например Mobileiron, AirWatch и др.');
                        }
                        ...
                        function sendRequestAndUpdateUI(value, item, techInfoId) {
                            return sendPatchRequest(value, item, techInfoId)
                                ...
                                .catch(error => {
                                    ...
                                    errorShowToast(`Ошибка при обновлении ${item}: ${error.message}`);
                                });
                        }
                        ...
                        ['api', 'localizable_web', 'localizable_ios', 'skins_web', 'skins_ios'].forEach(item => {
                            ...
                            if (elements[item]) {
                                ...
                                initializeSwitchery(`#${newCheckbox.id}`);
                            }
                        });
                    } else {
                        ...
                        .then(response => {
                            if (response.ok) {
                                // Если запрос выполнен успешно, показываем уведомление об успешном выполнении
                                successShowToast('Запрос на создание данных успешно выполнен');
                        ...
                        .catch(error => {
                            ...
                            console.error(error);
                            // Показываем уведомление об ошибке
                            errorShowToast(`Ошибка при создании данных: ' + error.message`);
                        });
                    }
                        ...
                        Promise.all([serverPromise, datePromise, ...patchPromises])
                            .then(responses => {
                                ...
                                if (allSuccessful) {
                                    ...
                                    successShowToast('Все запросы успешно выполнены');
                                    // ... потом добавлю другие действия после успешных запросов ...
                                } else {
                                    errorShowToast(`Произошла ошибка при выполнении запросов`);
